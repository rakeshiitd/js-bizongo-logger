...
(function(window, document) {
    ...
    window.messageFormatter = function(payload) {
        var ff = {};
        ff["message"] = payload[0]
    ...
    window.initializeLogging = function(nameOfApplication){
        debugger
        window.autoSettings();
        var _LTracker = _LTracker || [];
        _LTracker.push({'logglyKey': 'b83e39c3-4978-4bc2-9e55-73940f48a4a6',
        'sendConsoleErrors' : true,
        'tag' : 'nameOfApplication'  });
    };
    window.autoSettings = function(){
    Logger.useDefaults();
    Logger.setHandler(function (messages, context) {
    // Send messages to a custom logging endpoint for analysis.
    // TODO: Add some security? (nah, you worry too much! :P)
    debugger
    console.log("from logger",messages,context)
    payload = window.messageFormatter(messages);
    payload['level']=context.level.name.toLowerCase()
